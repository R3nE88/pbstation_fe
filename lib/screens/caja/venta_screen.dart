...
class _VentaScreenState extends State<VentaScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    void agregarPestania() {
      if (VentasStates.pestanias >= _maximoPestanias) {
        return;
      }
      setState(() {
    ...
    void selectedPestania(int index) {
      if (VentasStates.indexSelected == index) {
        return;
      }
      setState(() {
  ...
  VentasRecibidasButton ventaRecibida(
    BuildContext context,
    void Function() rebuild,
  ) {
    return VentasRecibidasButton(
      onPressed: () async {
        final ventasRecibida = Provider.of<VentasEnviadasServices>(
          context,
          listen: false,
        );
        int? seleccion;
              ...
              return Stack(
                ...
                children: [
                  AlertDialog(
                    ...
                    title: const Center(
                      child: Text(
                        'No se puede sobreescribir esta pestaña de venta',
                      ),
                    ),
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          DateTime dt = DateTime.parse(
                            ventasRecibida.ventas[index].fechaEnvio,
                          );
                          final DateFormat formatter = DateFormat('hh:mm:ss a');
                          final String formatted = formatter.format(dt);

                          String cliente = Provider.of<ClientesServices>(
                            context,
                            listen: false,
                          ).obtenerNombreClientePorId(
                            ventasRecibida.ventas[index].clienteId,
                          );
                          String usuario =
        ...
        if (!context.mounted) return;
        await showDialog(
          //TODO: Agregar opcion en ajustes para mostrar esta advertencia siempre, o no, solo en caja
          context: context,
          builder: (context) {
            return Stack(
              alignment: Alignment.topRight,
        ...
        if (continuar) {
          ...
          if (!context.mounted) return;
          final clientesS = Provider.of<ClientesServices>(
            context,
            listen: false,
          );
          final productosS = Provider.of<ProductosServices>(
            context,
            listen: false,
          );
          ...
          VentasStates.tabs[index].clienteSelected = clientesS.clientes
              .firstWhere((element) => element.id == venta.clienteId);
          VentasStates.tabs[index].entregaInmediata = !venta.hasPedido;
          VentasStates.tabs[index].fechaEntrega =
              venta.fechaEntrega != null
                  ? DateTime.parse(venta.fechaEntrega!)
                  : null;
          for (var detalle in venta.detalles) {
            VentasStates.tabs[index].productos.add(
              productosS.productos.firstWhere(
                (element) => element.id == detalle.productoId,
              ),
            );
          }
          ...
          VentasStates.tabs[index].fromVentaEnviadaData = {
            'id': venta.id!,
            'sucursal': venta.sucursalId,
          };
          VentasStates.tabs[index].comentariosController.text =
              venta.comentariosVenta ?? '';
          VentasStates.tabs[index].subtotalController.text = Formatos.pesos
              .format(venta.subTotal.toDouble());
          VentasStates.tabs[index].totalDescuentoController.text = Formatos
              .pesos
              .format(venta.descuento.toDouble());
          VentasStates.tabs[index].totalIvaController.text = Formatos.pesos
              .format(venta.iva.toDouble());
          VentasStates.tabs[index].totalController.text = Formatos.pesos.format(
            venta.total.toDouble(),
          );

          //Eliminar VentaRecibida
          ventasRecibida.ventas.removeWhere(
            (element) => element.id == venta.id,
          );
...
class Pestania extends StatelessWidget {
  const Pestania({
    ...
    this.agregarPestania,
    this.selectedPestania,
    this.rebuild,
    required this.index,
  });
        ...
        if (VentasStates.tabs[index].fromVentaEnviada) {
          ...
          if (success == true) {
            ...
            Provider.of<VentasEnviadasServices>(
              ...
              VentasStates.tabs[index].fromVentaEnviadaData['id']!,
              VentasStates.tabs[index].fromVentaEnviadaData['sucursal']!,
            );
            for (var pedidoId in VentasStates.tabs[index].pedidosIds) {
              Provider.of<PedidosService>(
                context,
                listen: false,
