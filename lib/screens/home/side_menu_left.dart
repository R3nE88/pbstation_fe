...
class SideMenuLeft extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Usamos Selector para escuchar solo el módulo seleccionado, no todo el provider
    return Selector<ModulosProvider, String>(
      selector: (_, p) => p.moduloSeleccionado,
      builder: (context, moduloSeleccionado, _) {
        final modProv = context.read<ModulosProvider>();
        final modulos = modProv.todosLosModulos;
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    final bloqueado = modProv.moduloBloqueado(modulo);
                    final selected = moduloSeleccionado == modulo.nombre;

                    return _navItem(
                      icon: bloqueado ? Icons.lock : modulo.iconoPrincipal,
                      label: modulo.nombre,
                      selected: selected,
                      onTap:
                          bloqueado
                              ? () {}
                              : () => modProv.seleccionarModulo(modulo.nombre),
                      index: i,
    ...
    return MouseRegion(
      cursor:
          inhabilitado
              ...
              : SystemMouseCursors.click, // ← CAMBIAR
      child: GestureDetector(
