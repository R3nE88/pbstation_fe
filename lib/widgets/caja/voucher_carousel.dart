...
class _VoucherBoardState extends State<VoucherBoard> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Agregar el listener de teclado solo después de que el árbol de widgets esté completamente construido
  ...
  @override
  void dispose() {
    // Eliminar el listener de teclado al destruir el widget
    RawKeyboard.instance.removeListener(_handleRawKey);
    super.dispose();
  }

  void _handleRawKey(RawKeyEvent event) {
    if (event is RawKeyDownEvent && widget.focusButton.hasFocus) {
  ...
  void _notifyControllers() {
    ...
    final allControllers = [...debitoControllers, ...creditoControllers];
    widget.onControllersChanged?.call(allControllers);
  }
            ...
            DirectionIntent: CallbackAction<DirectionIntent>(
              onInvoke: (intent) {
                ...
                if (intent.direction == 'down' || (intent.direction == 'enter' && !widget.focusButton.hasFocus)) {
                  ...
                  if (_creditoKey.currentState?.hasFocus() == true) {
                    _debitoKey.currentState?.focusCurrent();
